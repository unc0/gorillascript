(function (GLOBAL) {
  ...
  child_process.exec("which gjs", (_once = false, function (err, whichGjsStdout, whichGjsStderr) {
    ...
    optimist = require("optimist").usage("$0 [OPTIONS] path/to/script.gs", {
      ...
      commonjs: {
        boolean: true,
        desc: "Join all the generated CommonJS modules into a signle file which can be used in browser"
      }
    });
    if (hasGjs) {
    ...
    optimist.check(function (argv) {
      ...
      if (argv._.length > 1 && argv.map && !argv.join && !argv.commonjs) {
      ...
      function _step(_received) {
        while (true) {
          ...
          case 20:
            ...
            _state = filenames.length > 1 && argv.join || filenames.length > 0 && argv.commonjs ? 21 : 23;
            break;
          case 21:
            _arr = [];
